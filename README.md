...
This tool works by cycling through HTTP/2 stream ID numbers to avoid terminating its initial TCP connection, allowing up to 32767 ((2**16)/2 - 1) requests being sent before a connection restart.
...
Due to being able to maintain concurrency, tests indicate this tool can send up to ~1000-2000 requests per second.
